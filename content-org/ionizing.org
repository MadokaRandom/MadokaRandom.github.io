...
- [[#posts][Posts]]
  ...
  - [[#组内集群折腾实录][组内集群折腾实录]]
  - [[#proj-vasp-偶极校正及功函数的计算][PROJ VASP 偶极校正及功函数的计算]]
...
** TODO 组内集群折腾实录
** PROJ VASP 偶极校正及功函数的计算 :@PhysicalChemistry:VASP:Python:ASE:DipoleCorrection:Workfunction:
:PROPERTIES:
...
过偶极校正后，功函数会出现一个阶梯，阶梯两侧附近的曲接近水平。
...
OUTCAR 中的 E-fermi 才是实验中测得的真空能级的值。所幸的是 plot-workfunc.py 已经
