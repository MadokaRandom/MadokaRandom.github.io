...
\scalebox{2.0}{
    \begin{tikzpicture}[thick]
        \def\z{3}
        % Axes
        \draw[-stealth] (-\z-1/2,0) -- (\z+1/2,0) node[above left] {$z$};
        \draw[thick,dashed] (0,-1) -- (0,2);

        % Label charges
        \draw[draw=black!0, fill=gray!50] (-\z/4*3,1) circle (0.15)
        \draw[draw=black!0, fill=gray!50] (\z/4*3,1) circle (0.15)

        % Dashed line between two charges
        \draw[dashed] (-\z/4*3,1) node[above left] {$-\beta e$} -- (\z/4*3,1) node[above right] {$e$}
    \end{tikzpicture}
}
#+end_src
...
这里复用一下解氢原子的 Schrodinger 方程时用到的径向方程：
